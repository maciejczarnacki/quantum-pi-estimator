...
NUMPOINT = int(input("Input the number of darts to be simulated"))
PRECISION = int(input("Input the no of division to be divided the edge of the board"))
...
for i in range(NUMPOINT):
    ...
    x_points.append(x_dec)
    y_points.append(y_dec)
    # Checking the points inside the circle
    if ((x_dec - 0.5) ** 2 + (y_dec - 0.5) ** 2) <= 0.25:
        k += 1
...
circle = plt.Circle((0.5, 0.5), 0.5, color='blue', fill=False, linewidth=1)
plt.gca().add_patch(circle)
# Plot points
colors = ['green' if (x - 0.5) ** 2 + (y - 0.5) ** 2 <= .25 else 'red' for x, y in zip(x_points, y_points)]
